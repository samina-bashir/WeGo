...
import { collection, query, where, doc, getDocs, getDoc, onSnapshot, setDoc, updateDoc, documentId } from 'firebase/firestore';
...
const FindHostScreen = () => {
    ...
    const chechkWayPoints = (fromLat, toLat, fromLong, toLong, routeTime, onSuccess) => {
        try {
    ...
    const fetchRequests = async () => {
        ...
        for (const document of querySnapshot.docs) {
            ...
            if (requestData?.from?.latitude && requestData?.to?.latitude, requestData?.from?.longitude && requestData?.to?.longitude && requestData?.routeTime) {
                console.log('request curr from: ', requestData?.from?.name)
                console.log('request curr to: ', requestData?.to?.name)
                chechkWayPoints(requestData.from.latitude, requestData.to.latitude, requestData.from.longitude, requestData.to.longitude, requestData.routeTime, async (extraTime) => {
                    // alert('almost ')
                    const createdBy = requestData?.createdBy;
                    console.log('ok', createdBy)
                    // alert('started '+ createdBy)
                    ...
                    //sort filter 
                    const filtered = requestsData.sort((a, b) => {
